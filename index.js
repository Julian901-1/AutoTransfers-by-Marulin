  ...
  try {
    ...
    alfaSmsQueueChecker = setInterval(() => {
      ...
      if (pendingType === 'alfa_sms') {
        ...
        if (queuedSMS) {
          console.log(`[CHECKER-ALFA-EVENING] ✅ Found SMS code in queue (key: ${usedKey}), submitting...`);

          if (Date.now() < queuedSMS.expiresAt) {
            const submitted = alfaAutomation.submitAlfaSMSCode(queuedSMS.code);

            if (submitted) {
              console.log(`[CHECKER-ALFA-EVENING] ✅ Code submitted successfully`);
              smsCodeQueue.delete(usedKey);
            } else {
              console.log(`[CHECKER-ALFA-EVENING] ⚠️ Code submission returned false (will retry)`);
            }
          } else {
  ...
  try {
    ...
    alfaSmsQueueChecker = setInterval(() => {
      ...
      if (pendingType === 'alfa_sms') {
        ...
        if (queuedSMS) {
          console.log(`[CHECKER-ALFA-STAGE1] ✅ Found SMS code in queue (key: ${usedKey}), submitting...`);

          if (Date.now() < queuedSMS.expiresAt) {
            const submitted = alfaAutomation.submitAlfaSMSCode(queuedSMS.code);

            if (submitted) {
              console.log(`[CHECKER-ALFA-STAGE1] ✅ Code submitted successfully`);
              smsCodeQueue.delete(usedKey);
            } else {
              console.log(`[CHECKER-ALFA-STAGE1] ⚠️ Code submission returned false (will retry)`);
            }
          } else {
  ...
  try {
    ...
    alfaSmsQueueChecker = setInterval(() => {
      ...
      if (pendingType === 'alfa_sms') {
        ...
        if (queuedSMS) {
          console.log(`[CHECKER-ALFA-STAGE2] ✅ Found SMS code in queue (key: ${usedKey}), submitting...`);

          if (Date.now() < queuedSMS.expiresAt) {
            const submitted = alfaAutomation.submitAlfaSMSCode(queuedSMS.code);

            if (submitted) {
              console.log(`[CHECKER-ALFA-STAGE2] ✅ Code submitted successfully`);
              smsCodeQueue.delete(usedKey);
            } else {
              console.log(`[CHECKER-ALFA-STAGE2] ⚠️ Code submission returned false (will retry)`);
            }
          } else {
